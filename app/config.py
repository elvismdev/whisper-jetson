...
class CONFIG:
    ...
    DEVICE = os.getenv("ASR_DEVICE", "cuda" if (torch is not None and torch.cuda.is_available()) else "cpu")
    ...
    MODEL_QUANTIZATION = os.getenv("ASR_QUANTIZATION", "float32" if (torch is not None and torch.cuda.is_available()) else "int8")
